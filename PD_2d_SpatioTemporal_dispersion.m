...
kappa = linspace(1e-4,kappa_range,kappa_num);
k = kappa/h;

% omega0 is the exact angular frequency obtained from the discrete system.
omega0p = zeros(length(theta_set),length(kappa));
omega0s = zeros(length(theta_set),length(kappa));

for iii = 1:length(theta_set)
    theta = theta_set(iii);

    for jjj = 1:length(kappa)

        % kappa = kappa(jjj);     % kappa = k*Delta x
        ...
        U = zeros(n_node,1);
        for i = 1:n_node
            U(i,1) = exp(1i*k(jjj)*(normalized_coor(i,1)*cos(theta) + ...
        ...
        omega0p(iii,jjj) = sqrt((d1+d3)/2 + ...
                                sqrt(((d1-d3)/2)^2 + d2^2));

        omega0s(iii,jjj) = sqrt((d1+d3)/2 - ...
                                sqrt(((d1-d3)/2)^2 + d2^2));
    end
end

% Convert omega0 to exact wavenumber k0.
k0p = omega0p/vp_exact;
k0s = omega0s/vs_exact;
...
plot(kappa/pi,kappa0s(length(theta_set),:)/pi,'.b', ...
    'linewidth',2,'markersize',10); hold on
plot(kappa/pi,kappa0p(length(theta_set),:)/pi,'.r', ...
    'linewidth',2,'markersize',10); hold on

h2 = plot(kappa/pi,kappa0s(1,:)/pi,'-ob', ...
    'linewidth',2,'markerindices',(1:5:kappa_num), ...
    'markersize',5,'markerfacecolor','b'); hold on

h1 = plot(kappa/pi,kappa0p(1,:)/pi,'-or', ...
    'linewidth',2,'markerindices',(1:5:kappa_num), ...
...
for aaa = 3
    ...
    ghp_plot{1} = zeros(3,length(kappa));
    ghp_plot{2} = zeros(3,length(kappa));
    ghs_plot{1} = zeros(3,length(kappa));
    ghs_plot{2} = zeros(3,length(kappa));

    ep{1} = zeros(3,length(kappa));
    ep{2} = zeros(3,length(kappa));
    es{1} = zeros(3,length(kappa));
    es{2} = zeros(3,length(kappa));
    ...
    for theta_ind = 1:length(theta_set)
        for wave_ind = 1:2
            ...
            if wave_ind == 1
                % P-wave
                k0 = kappa0p(theta_ind,:)/h;   % exact wavenumber
                c0 = vp_exact;                      % exact P-wave speed
            else
                % S-wave
                k0 = kappa0s(theta_ind,:)/h;   % exact wavenumber
                    ...
                    for iii = 1:3
                        ...
                        if wave_ind == 1
                            ...
                            kappa_discard{theta_ind}(1,iii) = ...
                                NB_find_discard_kappa(e,kappa,c0,CFL);
                        else
                            es{theta_ind}(iii,:) = e;
    ...
    yyaxis right
    plot(kappa/pi,ghp_plot{length(theta_set)}(2,:),'-o','color',[0.5 0.5 1], ...
        'markerindices',(1:5:kappa_num),'markersize',5, ...
        'markerfacecolor',[0.5 0.5 1],'linewidth',2); hold on
    plot(kappa/pi,ghp_plot{length(theta_set)}(3,:),'-o','color',[1 0.5 0.5], ...
        'markerindices',(1:5:kappa_num),'markersize',5, ...
        'markerfacecolor',[1 0.5 0.5],'linewidth',2);
    plot(kappa/pi,ghp_plot{length(theta_set)}(1,:),'-o','color',[0.5 0.5 0.5], ...
        'markerindices',(1:5:kappa_num),'markersize',5, ...
        'markerfacecolor',[0.5 0.5 0.5],'linewidth',2);
    plot(kappa/pi,ghp_plot{1}(2,:),'-ob','markerindices',(1:5:kappa_num), ...
        'markersize',5,'markerfacecolor','b','linewidth',2);
    plot(kappa/pi,ghp_plot{1}(3,:),'-or','markerindices',(1:5:kappa_num), ...
        'markersize',5,'markerfacecolor','r','linewidth',2);
    plot(kappa/pi,ghp_plot{1}(1,:),'-ok','markerindices',(1:5:kappa_num), ...
    ...
    yyaxis left
    plot(kappa(kappa<kappa_discard{2}(1,1))/pi, ...
        ep{2}(1,kappa<kappa_discard{2}(1,1)), ...
        '-','color',[0.5 0.5 0.5],'linewidth',2); hold on
    plot(kappa(kappa<kappa_discard{2}(1,2))/pi, ...
        ep{2}(2,kappa<kappa_discard{2}(1,2)), ...
        '-','color',[0.5 0.5 1],'linewidth',2);
    plot(kappa(kappa<kappa_discard{2}(1,3))/pi, ...
        ep{2}(3,kappa<kappa_discard{2}(1,3)), ...
    ...
    h1 = plot(kappa(kappa<kappa_discard{1}(1,1))/pi, ...
        ep{1}(1,kappa<kappa_discard{1}(1,1)), ...
        '-k','linewidth',2);
    h2 = plot(kappa(kappa<kappa_discard{1}(1,2))/pi, ...
        ep{1}(2,kappa<kappa_discard{1}(1,2)), ...
        '-b','linewidth',2);
    h3 = plot(kappa(kappa<kappa_discard{1}(1,3))/pi, ...
        ep{1}(3,kappa<kappa_discard{1}(1,3)), ...
        '-r','linewidth',2);

    plot(kappa(kappa>kappa_discard{1}(1,1))/pi, ...
        ep{1}(1,kappa>kappa_discard{1}(1,1)), ...
        '.k','linewidth',2,'markersize',10);
    plot(kappa(kappa>kappa_discard{1}(1,2))/pi, ...
        ep{1}(2,kappa>kappa_discard{1}(1,2)), ...
        '.b','linewidth',2,'markersize',10);
    plot(kappa(kappa>kappa_discard{1}(1,3))/pi, ...
        ep{1}(3,kappa>kappa_discard{1}(1,3)), ...
    ...
    yyaxis right
    plot(kappa/pi,ghs_plot{length(theta_set)}(2,:),'-o','color',[0.5 0.5 1], ...
        'markerindices',(1:5:kappa_num),'markersize',5, ...
        'markerfacecolor',[0.5 0.5 1],'linewidth',2); hold on
    plot(kappa/pi,ghs_plot{length(theta_set)}(3,:),'-o','color',[1 0.5 0.5], ...
        'markerindices',(1:5:kappa_num),'markersize',5, ...
        'markerfacecolor',[1 0.5 0.5],'linewidth',2);
    plot(kappa/pi,ghs_plot{length(theta_set)}(1,:),'-o','color',[0.5 0.5 0.5], ...
        'markerindices',(1:5:kappa_num),'markersize',5, ...
        'markerfacecolor',[0.5 0.5 0.5],'linewidth',2);
    plot(kappa/pi,ghs_plot{1}(2,:),'-ob','markerindices',(1:5:kappa_num), ...
        'markersize',5,'markerfacecolor','b','linewidth',2);
    plot(kappa/pi,ghs_plot{1}(3,:),'-or','markerindices',(1:5:kappa_num), ...
        'markersize',5,'markerfacecolor','r','linewidth',2);
    plot(kappa/pi,ghs_plot{1}(1,:),'-ok','markerindices',(1:5:kappa_num), ...
        'markersize',5,'markerfacecolor','k','linewidth',2); hold off
    ...
    yyaxis left
    plot(kappa(kappa<kappa_discard{2}(2,1))/pi, ...
        es{2}(1,kappa<kappa_discard{2}(2,1)), ...
        '-','color',[0.5 0.5 0.5],'linewidth',2); hold on
    plot(kappa(kappa<kappa_discard{2}(2,2))/pi, ...
        es{2}(2,kappa<kappa_discard{2}(2,2)), ...
        '-','color',[0.5 0.5 1],'linewidth',2);
    plot(kappa(kappa<kappa_discard{2}(2,3))/pi, ...
        es{2}(3,kappa<kappa_discard{2}(2,3)), ...
        '-','color',[1 0.5 0.5],'linewidth',2);
    ...
    h1 = plot(kappa(kappa<kappa_discard{1}(2,1))/pi, ...
        es{1}(1,kappa<kappa_discard{1}(2,1)), ...
        '-k','linewidth',2);
    h2 = plot(kappa(kappa<kappa_discard{1}(2,2))/pi, ...
        es{1}(2,kappa<kappa_discard{1}(2,2)), ...
        '-b','linewidth',2);
    h3 = plot(kappa(kappa<kappa_discard{1}(2,3))/pi, ...
        es{1}(3,kappa<kappa_discard{1}(2,3)), ...
        '-r','linewidth',2);

    plot(kappa(kappa>kappa_discard{1}(2,1))/pi, ...
        es{1}(1,kappa>kappa_discard{1}(2,1)), ...
        '.k','linewidth',2,'markersize',10);
    plot(kappa(kappa>kappa_discard{1}(2,2))/pi, ...
        es{1}(2,kappa>kappa_discard{1}(2,2)), ...
        '.b','linewidth',2,'markersize',10);
    plot(kappa(kappa>kappa_discard{1}(2,3))/pi, ...
        es{1}(3,kappa>kappa_discard{1}(2,3)), ...
        '.r','linewidth',2,'markersize',10); hold off
