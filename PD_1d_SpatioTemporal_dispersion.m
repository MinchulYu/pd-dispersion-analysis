...
omega0=zeros(1,length(kappa));
k = kappa/h;
for i=1:length(kappa)
    U = zeros(Nx,1);
    for j = 1:Nx
        U(j,1) = exp(1i*k(i)*(normalized_coord(j)));
    end
    d1 = real((U'*K*U)/(U'*M*U));
...
plot(kappa/pi,kappa0/pi,'-b','linewidth',2); hold on
...
for aaa = 1
    ...
    gh_plot = zeros(3,length(kappa));
    e_all = zeros(3,length(kappa));
    ...
    yyaxis left
    plot(kappa(kappa<kappa_discard(1,1))/pi, ...
        e_all(1,kappa<kappa_discard(1,1)), ...
        '-k','linewidth',2); hold on
    plot(kappa(kappa<kappa_discard(1,2))/pi, ...
        e_all(2,kappa<kappa_discard(1,2)), ...
        '-b','linewidth',2); hold on
    plot(kappa(kappa<kappa_discard(1,3))/pi, ...
        e_all(3,kappa<kappa_discard(1,3)), ...
